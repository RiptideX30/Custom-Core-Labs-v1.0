...
const NEW_BUILDS = [
  { id: "basic", title: "Basic Build", price: 0, priceLabel: "15% of parts", desc: "Pure hardware assembly. No OS or drivers." },
  { id: "ultimate", title: "Ultimate Build", price: 139, priceLabel: "$139 flat · <$1,500", desc: "Full assembly, OS provisioning, pro routing, stress test." },
] as const;
...
function LiveEstimator({
  ...
  const lineItems = useMemo(() => {
    ...
    if (services.has("basic")) {
      items.push({ label: "Basic Build · 15% of parts", amount: +(partsValue * 0.15).toFixed(2) });
    }
    if (services.has("ultimate")) {
      if (partsValue > 1500) {
        items.push({ label: "Ultimate Build · 8% of parts", amount: +(partsValue * 0.08).toFixed(2) });
        items.push({ label: "Software surcharge (>$1,500)", amount: 49 });
      } else {
        items.push({ label: "Ultimate Build · flat", amount: 139 });
      }
    }
